...
public class EditGoodsDialog extends JDialog {
    private User user;
    private GoodsService service = new GoodsService();
    private Goods goods;

    public EditGoodsDialog(JFrame owner, User user, Goods goods) {
        super(owner, "编辑物资", true);
        this.user = user;
        this.goods = goods;
        ...
        typeField.setText(goods.getType());
        ...
        gbc.gridx = 1;
        JSpinner minQuantitySpinner = new JSpinner(new SpinnerNumberModel(goods.getMinQuantity().intValue(), 1, 1000, 1));
        ...
        saveButton.addActionListener(event -> handleSave(nameField, typeField, unitField, minQuantitySpinner));
        cancelButton.addActionListener(event -> dispose());

        setTitle("编辑物资 - " + goods.getName());
        setSize(500, 300);
        setLocationRelativeTo(owner);
        setContentPane(rootPanel);
    }

    private void handleSave(JTextField nameField, JTextField typeField, JTextField unitField, JSpinner minQuantitySpinner) {
        ...
        try {
            service.update(goods.getId(), name, type, unit, minQuantity);
            MessageUtil.info(this, "物资更新成功");
            dispose();
