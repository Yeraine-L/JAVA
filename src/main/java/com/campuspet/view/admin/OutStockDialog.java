...
public class OutStockDialog extends JDialog {
    private User user;
    private GoodsService service = new GoodsService();

    public OutStockDialog(JFrame owner, User user) {
        super(owner, "物资出库", true);
        this.user = user;
        ...
        outButton.addActionListener(event -> handleOutStock(goodsCombo, quantitySpinner, remarkField));
        cancelButton.addActionListener(event -> dispose());

        setTitle("物资出库 - " + user.getUsername());
        setSize(500, 250);
    ...
    private void handleOutStock(JComboBox<Goods> goodsCombo, JSpinner quantitySpinner, JTextField remarkField) {
        ...
        try {
            ...
            service.outStock(goods.getId(), quantity, user, remark);
            MessageUtil.info(this, "物资出库成功");
            dispose();
